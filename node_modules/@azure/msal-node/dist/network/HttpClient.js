    ...
    return new Promise((resolve, reject) => {
        ...
        request.on("connect", (response, socket) => {
            ...
            socket.on("end", () => {
                ...
                if ((httpStatusCode < Constants.HttpStatus.SUCCESS_RANGE_START ||
                    httpStatusCode > Constants.HttpStatus.SUCCESS_RANGE_END) &&
    ...
    return new Promise((resolve, reject) => {
        ...
        request.on("response", (response) => {
            ...
            response.on("end", () => {
                ...
                if ((statusCode < Constants.HttpStatus.SUCCESS_RANGE_START ||
                    statusCode > Constants.HttpStatus.SUCCESS_RANGE_END) &&
                    // do not destroy the request for the device code flow
                    networkResponse.body["error"] !==
                        Constants.Constants.AUTHORIZATION_PENDING) {
                    request.destroy();
...
const parseBody = (statusCode, statusMessage, headers, body) => {
    ...
    try {
        ...
        if (statusCode >= Constants.HttpStatus.CLIENT_ERROR_RANGE_START &&
            statusCode <= Constants.HttpStatus.CLIENT_ERROR_RANGE_END) {
            errorType = "client_error";
            errorDescriptionHelper = "A client";
        }
        else if (statusCode >= Constants.HttpStatus.SERVER_ERROR_RANGE_START &&
            statusCode <= Constants.HttpStatus.SERVER_ERROR_RANGE_END) {
